...
namespace AsterNET.Test
{
    class Program
    {
        const string DEV_HOST = "192.168.1.2";
        const int ASTERISK_PORT = 5038;
        const string ASTERISK_HOST = "192.168.1.2";
        ...
        static void checkLoginData()
        {
            ...
            while (string.IsNullOrWhiteSpace(PasswordManager.GetPassword("Zycoo.Ami.Login")))
            {
                Console.WriteLine("Zycoo.Ami.Login please :");
                PasswordManager.SetPassword("Zycoo.Ami.Login", Console.ReadLine());
            }

            while (string.IsNullOrWhiteSpace(PasswordManager.GetPassword("Zycoo.Ami.Password")))
            {
                Console.WriteLine("Zycoo.Ami.Password please :");
                PasswordManager.SetPassword("Zycoo.Ami.Password", Console.ReadLine());
            }
        ...
        {
            PasswordManager.DeletePassword("Zycoo.Ami.Login");
            PasswordManager.DeletePassword("Zycoo.Ami.Password");
        }
...
Ctrl-C to exit");
        ...
        {
            manager = new ManagerConnection(ASTERISK_HOST, ASTERISK_PORT
                , PasswordManager.GetPassword("Zycoo.Ami.Login")
                , PasswordManager.GetPassword("Zycoo.Ami.Password"));
        ...
        private static void checkManagerAPI()
        {
            ...
            try
            {
                manager.Login(ORIGINATE_TIMEOUT);           // Login only (fast)
